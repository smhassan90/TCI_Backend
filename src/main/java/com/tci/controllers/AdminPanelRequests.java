...
public class AdminPanelRequests {
    ...
    public String getAdminDropDownData(String tableName, String column1, String column2){
        ...
        String[] splittedColumns = column2.split("&");
        String columns = "";
        for(int i = 0; i<splittedColumns.length; i++){
            if(!columns.equals("")){
                columns+=",' - ',";
            }
            columns += splittedColumns[i] ;
        }
        String query = "SELECT "+column1+" AS key, CONCAT("+columns+") AS value FROM " +tableName + " order by " + column1;
